...
export const deleteFileFormCloudinary = async (url: string) => {
  ...
  if (!url || typeof url !== "string") {
    throw new AppError(StatusCodes.BAD_REQUEST, "File URL is required and must be a string");
  }
  ...
  const regex = /\/v\d+\/(.*?)\.(jpg|jpeg|png|gif|webp|mp4|mov|avi|mkv|pdf|doc|docx)$/i;
  ...
  try {
    ...
    if (res.result !== "ok" && res.result !== "not found") {
      throw new AppError(StatusCodes.INTERNAL_SERVER_ERROR, `Failed to delete file: ${res.result}`);
    }
...
export const uploadFileToCloudinary = async (file: Express.Multer.File, folder: string): Promise<UploadApiResponse> => {
  try {
    return new Promise((resolve, reject) => {
    ...
    throw new AppError(StatusCodes.INTERNAL_SERVER_ERROR, `Failed to upload file to Cloudinary. ${error.message}`);
  }
};
